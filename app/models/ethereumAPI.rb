...
class EthereumAPI
  ...
  def deployERC20Token(owner_address, ether_accpunt_password, name, symbol, totalTokens)
    ...
    @client.personal_unlock_account(owner_address, ether_accpunt_password)
  ...
  def executeTransfer(maker_address, to_username, amount, password)
    # owner_address = "0xd34da9604e5e9c2a9cc0aa481b6b24a72af3253b"
    @client = Ethereum::HttpClient.new('http://localhost:8545')
    @client.personal_unlock_account(to_username, password)
    @contract = Ethereum::Contract.create(file: PATH, client: @client)
